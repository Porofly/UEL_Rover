...
송신기 채널 방향은 기본값(Reverse 꺼짐) 기준이다: 스틱 오른쪽 / 위 = 펄스폭 증가 =
우회전 / 전진. 방향이 반대로 나오면 송신기의 `Reverse` 설정부터 확인한다.
...
| 그 밖 전부 (기본 위치 ≥ 1700 µs 포함) | STOP | 즉시 정지. 수신기 신호가 오기 전의 초기값도 STOP. |

스위치 순서는 STOP → AUTO → RC 다. RC 와 STOP 사이를 오갈 때 AUTO 를 거친다.
